...
TeXSplit ist ein nativer LaTeX-Editor für macOS. Die App kombiniert einen schnellen `NSTextView`-Codeeditor mit Zeilennummern, Syntax-Highlighting, Tabs und einer PDFKit-Vorschau für kompiliertes LaTeX.
...
- `.tex`-Dateien öffnen, bearbeiten, speichern und speichern unter
...
TeXSplit ist eine lokale macOS-Entwickler-App. Der Editor, Tabs, Syntax-Highlighting, Theme-Wechsel, Dateioperationen, PDF-Vorschau und automatische Kompilierung sind implementiert. Für die LaTeX-Kompilierung wird ein vorhandenes MacTeX/BasicTeX verwendet oder optional eine vorbereitete TeXLive-Runtime im App-Bundle.

## Voraussetzungen

- macOS 14 oder neuer
- Xcode 15 oder neuer
- Für lokale Entwicklerbuilds: MacTeX oder BasicTeX, falls keine Runtime im Bundle liegt
...
Wenn dort kein Compiler liegt, sucht die App für Entwicklerbuilds nach:

```sh
/Library/TeX/texbin/pdflatex
```

Danach wird `pdflatex` über `/usr/bin/env pdflatex` versucht, wenn eine lokale Installation im Suchpfad liegt.

## Starten

1. `TeXSplit.xcodeproj` in Xcode öffnen.
2. Scheme `TeXSplit` auswählen.
3. Run drücken.
...
- `TeXSplit/App`: App-Einstieg und native Menübefehle
- `TeXSplit/Models`: Dokument-, Theme-, Tab- und Statusmodelle
- `TeXSplit/ViewModels`: Editor- und Workspace-Logik
- `TeXSplit/Views`: SwiftUI-Oberfläche, AppKit-Editor und PDFKit-Vorschau
- `TeXSplit/Services`: Dateioperationen, Compilerstart, Fehlerparser, Syntax-Highlighting und Themes
- `TeXSplitTests`: Unit-Tests für Editor, Compiler, Parser, Tabs und Einstellungen

## TeXLive-Runtime

Der große TeXLive-Runtime-Ordner wird bewusst nicht ins Repository committed. Im Repository bleibt nur `TeXSplit/Resources/TeXLive/README.md`. Für lokale Tests oder Distributionsbuilds kann die Runtime separat vorbereitet werden:

```sh
./script/embed_basictex_runtime.sh
```

Das Skript lädt BasicTeX/TeXLive, extrahiert die Runtime und legt sie lokal unter `TeXSplit/Resources/TeXLive` ab. Dieser Ordner ist in `.gitignore` ausgeschlossen.
...
- Keine Projektordner-Verwaltung für größere Multi-File-LaTeX-Projekte
